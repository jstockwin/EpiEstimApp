...
shinyUI(bootstrapPage(theme = shinytheme("spacelab"),
  ...
  column(12,
         HTML(
           "
          <h1>EpiEstim App</h1>
          <p>This application relies on MCMC sampling to create a posterior estimate of disease transmissibility (i.e. the reproduction number, R) over time
          from a time series of daily incidence data, time-censored symptom onset data that are appropriate for constructing the serial interval (i.e. raw
          exposure data or a posterior sample of the serial interval), and, where applicable, a prior distribution for the serial interval.</p>
          ...
          <p>The authors request users to cite the original publication (TODO LINK) when referring to this tool, the format, or when
          reporting results generated by this tool.</p>
